...
void *philosopher(void *thread_num)
{
    ...
    for (int i = 0; i < EAT_TIMES; i++)
    {
        ...
        bool isPickedRightChop, isPickedLeftChop;
        if (id == 0)
        {
            isPickedRightChop = pthread_mutex_lock(&chopsticks[right]) == 0;
            isPickedLeftChop = pthread_mutex_lock(&chopsticks[left]) == 0;
        }
        else
        {
            isPickedRightChop = pthread_mutex_lock(&chopsticks[left]) == 0;
            isPickedLeftChop = pthread_mutex_lock(&chopsticks[right]) == 0;
        }

        if (isPickedLeftChop)
            printf("\nFilosofo %d: ha la sua bacchetta sinistra", id + 1);
        if (isPickedRightChop)
            printf("\nFilosofo %d: ha la sua bacchetta destra", id + 1);
        if (isPickedLeftChop && isPickedRightChop)
            printf("\nFilosofo %d: sta mangiando", id + 1);

        int isUnlockLeftChop = pthread_mutex_unlock(&chopsticks[left]);
        int isUnlockRightChop = pthread_mutex_unlock(&chopsticks[right]);
        if (isUnlockLeftChop && isUnlockRightChop)
